	...
	return (
		<div className="space-y-6" id="docs-content">
			...
			<div className="min-w-0">
				<div className="flex gap-2">
					...
					<div className="min-w-0">
						...
						<div className=" text-gray-500 text-sm">
							...
							{info.version && (
								<span
									className="inline-flex items-center rounded-full bg-indigo-50 px-1.5 py-0.5 text-xs font-medium text-indigo-700 ring-1 ring-inset ring-indigo-700/10 ml-1">
                  {info.version}
                </span>
							)}
						</div>
				...
				<div className="mt-1.5 flex flex-row flex-wrap gap-x-3 gap-y-1 min-w-0 text-sm text-gray-500">
					<div className="flex-none flex items-center">
						<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5"
								 stroke="currentColor" className="mr-1 h-4 w-4 shrink-0 text-gray-500">
					...
					<div className="flex-none flex items-center">
						<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5"
								 stroke="currentColor" className="mr-1 h-4 w-4 shrink-0 text-gray-400">
							<path strokeLinecap="round" strokeLinejoin="round"
					...
					<a href={`https://bitol-io.github.io/open-data-contract-standard/${parsedData.apiVersion || 'latest'}/`}
						 className="flex-none flex items-center" target="_blank" rel="noopener noreferrer">
            <span
                            ...
                            {indent > 0 && indentSpaces}
                                ...
                                transform description
													...
													{property.customProperties && Array.isArray(property.customProperties) && property.customProperties.map((customProp, idx) => (
														<span key={idx}
																	className="inline-flex items-center rounded-md bg-yellow-50 px-2 py-1 text-xs font-medium text-yellow-800 ring-1 ring-inset ring-yellow-600/20 mr-1 mt-1">
					...
					<section>
						...
						<ul role="list"
							...
							{servers.map((server, index) => {
								...
								return (
									<li key={index} className="relative flex flex-col gap-x-6 gap-y-2 px-4 py-3 sm:px-6"
																	...
																	const tooltipContent = (
																		<div className="space-y-1 text-xs">
																			...
																			{role.firstLevelApprovers &&
																				<div className="text-gray-300">1st Approvers: {role.firstLevelApprovers}</div>}
																			{role.secondLevelApprovers &&
																				<div className="text-gray-300">2nd Approvers: {role.secondLevelApprovers}</div>}
																		</div>
																	);
					...
					<section>
						<div className="px-4 sm:px-0">
							<h1 className="text-base font-semibold leading-6 text-gray-900" id="support">Support & Communication
								Channels</h1>
							<p className="text-sm text-gray-500">Support and communication channels help consumers find help regarding
